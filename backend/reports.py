...
def build_html_report(scan: ScanRecord) -> str:
    ...
    for finding in scan.findings:
        ...
        rows.append(
            ...
            f"<td>{finding.reach_score}</td>"
            f"<td>{finding.anomaly_score}</td>"
            f"<td>{rep_str}</td>"
            f"<td>{cat_str}</td>"
...
<body>
  <div class="shell">
    <section class="hero">
      <div class="eyebrow">ManifestGuard v4</div>
...
def write_pdf_report(scan: ScanRecord, destination: Path) -> Path:
    ...
    for idx, finding in enumerate(scan.findings):
        ...
        pdf._table_row(
            ...
            [
                ...
                str(finding.reach_score),
                str(finding.anomaly_score),
    ...
    if high_risk:
        ...
        for finding in high_risk:
            ...
            pdf._key_value("Reach Score", f"{finding.reach_score}/100", indent=10)
            pdf._key_value("Anomaly Score", f"{finding.anomaly_score}/100", indent=10)
    ...
    if trusted:
        ...
        for finding in trusted:
            pdf._body_line(f"  {finding.name} (Reach: {finding.reach_score})", indent=10, size=9)
    ...
    pdf._y -= 14
    pdf._body_line("Generated by ManifestGuard v4 - Evidence-Driven Extension Auditor", size=8)
